...
| `agent-retry-guard` | `packages/agent-retry-guard` → `@j1nn0/agent-retry-guard` `0.1.1` | `packages/agent-retry-guard-pi` → `@j1nn0/agent-retry-guard-pi` `0.2.0` | Published |
...
`0.1.1` so the already-published Context Guard release remains the safe default.
For the current Retry Guard manifests, select family `agent-retry-guard` with `core_version=0.1.1` and `pi_version=0.2.0`. The workflow defaults remain `0.1.1` so the already-published Context Guard release remains the safe default.
